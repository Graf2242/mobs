...
public class RangeCheckMicroBenchmark {
    ...
    private static void deoptimize(List<Integer> list) {
        for (Integer x : list)
            if (x == null)
                throw new Error();
    }
    ...
    public static void main(String[] args) throws Throwable {
        ...
        final List<Integer> list = new SimpleArrayList<Integer>(1000000);
        ...
        final Job[] jobs = {
                ...
                new Job("set") {
                    void work() {
                        Integer[] xs = (Integer[]) list.toArray(new Integer[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                list.set(k, xs[k]);
                        }
